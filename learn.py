...
import os, csv, json, html, urllib, base64, hashlib, argparse, platform, subprocess
...
from data import learn_base_url, default_headers

ssl._create_default_https_context = ssl._create_unverified_context
global dist_path, headers, cookie, opener, err404, fingerprint_data
dist_path = headers = cookie = opener = err404 = fingerprint_data = None
...
def open_page(uri, values={}):
    ...
    request = urllib.request.Request(
        uri if uri.startswith("http") else learn_base_url + uri, post_data, headers
    )
    try:
...
def get_page(uri, values={}, session=None):
    if session:
        # 使用新的session方式
        try:
            url_full = uri if uri.startswith("http") else learn_base_url + uri
            if values:
                # 使用与原始代码完全相同的编码方式
                import urllib.parse
...
def download(uri, name, target_dir=None, session=None):
    ...
    try:
        if session:
            # 使用新的session方式
            download_url = uri if uri.startswith("http") else learn_base_url + uri
...
def process_course(c, args):
    ...
    blm = BrowserLoginManager(headless=True)
